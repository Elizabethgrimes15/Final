...
this.Given(/^I am on Filter Check Demo Page$/, async () => {
 ...
 this.Then(/^I typing a "Tree"$/, async () => {
...
//Copy File
this.Given(/^I am on Data table Download Page$/, async () => {
   await seleniumEasyPage.goTodatatabledownloadpage();
});

 this.Then(/I type "Airi" into unput$/, async () => {
    await seleniumEasyPage.searchInputClick();
 });

  this.Then(/^press "Copy" button$/, async () => {
   await seleniumEasyPage.copyButtonPress();
});

this.Then(/^the success modal appears$/, async () => {
   await seleniumEasyPage.copyIntoClipboardModal();
});

//Searching through Data table
this.Given(/^I am on Data table Demo Page$/, async () => {
   await seleniumEasyPage.goTodatatablesearch();
});

 this.Then(/I typing request "Land" into a search input$/, async () => {
    await seleniumEasyPage.searchInputClickAndType();
 });

  this.Then(/^there is a match is displayed$/, async () => {
   await seleniumEasyPage.rowFilterVisibility();
});


//Filtering Data table
this.Given(/^I am on Data table Filtering Page$/, async () => {
   await seleniumEasyPage.goTodatatablesearch1();
});

 this.Then(/I click to "sorting filters" button$/, async () => {
    await seleniumEasyPage.sortingFiltersClick();
 });

  this.Then(/^I typing 'Jacobs' into a surname field$/, async () => {
   await seleniumEasyPage.surnameInputFieldType();
});
this.Then(/^there is a matching data table sorting is displayed$/, async () => {
   await seleniumEasyPage.rowResultsDataTable();
});

//PDF File Download
this.Given(/^I am on Data Page$/, async () => {
   await seleniumEasyPage.goToDataPage();
});

 this.Then(/I input "Chief" into a search field$/, async () => {
    await seleniumEasyPage.searchFieldInput();
 });

  this.Then(/^I click to PDF icon for file downloading$/, async () => {
   await seleniumEasyPage.pdfIconButtonClick();
});


//IFrame
this.Given(/^I am on IFrame Sample Page$/, async () => {
   await seleniumEasyPage.goToIframePage();
});

 this.Then(/I Switch to the Frame$/, async () => {
    await seleniumEasyPage.iframeSwitching();
 });

  this.Then(/^I see the heading "Nested Page Example"$/, async () => {
   await seleniumEasyPage.textExampleView();
});
