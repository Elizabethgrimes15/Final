...
//Copy File
public async goTodatatabledownloadpage() {
    await browser.navigate().to(browser.params.datatabledownloadpage);
}
public async searchInputClick(){
    await browser.wait(ExpectedConditions.visibilityOf(this.seleniumEasyRepo.inputsearching), defaultTimeout, "Time out");
    await browser.actions().mouseMove(this.seleniumEasyRepo.inputsearching).click().sendKeys('Airi').perform();
}
public async copyButtonPress(){
    await browser.wait(ExpectedConditions.visibilityOf(this.seleniumEasyRepo.copybutton), defaultTimeout, "Time out");
    await browser.actions().mouseMove(this.seleniumEasyRepo.copybutton).click().perform();
}
public async copyIntoClipboardModal(){
    await browser.wait(ExpectedConditions.visibilityOf(await this.seleniumEasyRepo.clipboardmodal), defaultTimeout);
}

//Searching through Data table
public async goTodatatablesearch() {
    await browser.navigate().to(browser.params.datatablesearch);
}
public async searchInputClickAndType(){
    await browser.wait(ExpectedConditions.visibilityOf(this.seleniumEasyRepo.inputtyping), defaultTimeout, "Time out");
    await browser.actions().mouseMove(this.seleniumEasyRepo.inputtyping).click().sendKeys('Land').perform();
}
public async rowFilterVisibility(){
    await browser.wait(ExpectedConditions.visibilityOf(await this.seleniumEasyRepo.rowfilter), defaultTimeout);
}

//Filtering Data table
public async goTodatatablesearch1() {
    await browser.navigate().to(browser.params.datatablesearch);
}
public async sortingFiltersClick(){
    await browser.wait(ExpectedConditions.visibilityOf(this.seleniumEasyRepo.sortingfilterselem), defaultTimeout, "Time out");
    await browser.actions().mouseMove(this.seleniumEasyRepo.sortingfilterselem).click().perform();
}
public async surnameInputFieldType(){

    await browser.wait(ExpectedConditions.visibilityOf(this.seleniumEasyRepo.surnameinput), defaultTimeout, "Time out");
    await browser.actions().mouseMove(this.seleniumEasyRepo.surnameinput).click().sendKeys('Jacobs').perform();
}
public async rowResultsDataTable(){
    await browser.wait(ExpectedConditions.visibilityOf(await this.seleniumEasyRepo.rowusershown), defaultTimeout);
}


//PDF File Download
public async goToDataPage() {
    await browser.navigate().to(browser.params.datatabledownloadpage);
}
public async searchFieldInput(){
    await browser.wait(ExpectedConditions.visibilityOf(this.seleniumEasyRepo.inputsearching), defaultTimeout, "Time out");
    await browser.actions().mouseMove(this.seleniumEasyRepo.inputsearching).click().sendKeys('Chief').perform();
}
public async pdfIconButtonClick(){
    await browser.wait(ExpectedConditions.visibilityOf(this.seleniumEasyRepo.pdficon), defaultTimeout, "Time out");
    await browser.actions().mouseMove(this.seleniumEasyRepo.pdficon).click().perform();
}

//IFrame
public async goToIframePage() {
    await browser.navigate().to(browser.params.iframesamplepage);
}
public async iframeSwitching(){
    await browser.wait(ExpectedConditions.visibilityOf(this.seleniumEasyRepo.iframeelem), defaultTimeout, "Time out");
    await browser.switchTo().frame(this.seleniumEasyRepo.iframeelem.getWebElement());
}
public async textExampleView(){
    await browser.wait(ExpectedConditions.visibilityOf(this.seleniumEasyRepo.iframetext), defaultTimeout, "Time out");
